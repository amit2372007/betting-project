...
mongoose.connection.once("open", async () => {
  ...
  try {
    ...
    const result1 = await db
      ...
      .deleteMany({ matchId: null });
    const result2 = await db
      .collection("exposures")
      .deleteMany({ eventId: null });

    if (result1.deletedCount > 0 || result2.deletedCount > 0) {
      console.log(
        `✅ SUCCESS: Deleted ${result1.deletedCount + result2.deletedCount} corrupted exposure documents.`,
      );
    }
  } catch (e) {
  ...
  try {
    ...
    if (event) {
      ...
      if (event.matchOdds && event.matchOdds.status === "suspended") {
        ...
        return res
          .status(200)
          .send("Market is suspended by Admin. Odds ignored.");
      }
...
// Quick Express redirect example
app.get("/join/:promoCode", async (req, res) => {
  const { promoCode } = req.params;

  try {
    // 1. Fetch the active WhatsApp document
    ...
    if (!WA) {
      console.error("No active WhatsApp deposit number found for redirect.");
      return res.redirect("/");
    }

    // 2. Extract and CLEAN the number
    let activeNumber = WA.phoneNumber;
    activeNumber = activeNumber.replace(/\D/g, "");

    // 3. TRACK THE CLICK FIRST (CRITICAL FIX)
    // We clean the promo code just like we did in the Add form to guarantee a match
    const cleanPromo = promoCode.trim().toUpperCase().replace(/\s+/g, "");

    // USE REGEX TO BYPASS INVISIBLE CHARACTERS
    const influencer = await Influencer.findOne({
      promoCode: new RegExp(`^${cleanPromo}$`, "i"),
    });

    if (influencer) {
      influencer.totalClicks += 1;
      await influencer.save(); // Node.js waits for MongoDB to confirm the save
    } else {
      console.log(`Notice: Promo code '${cleanPromo}' not found in database.`);
    }

    // 4. SEND THE REDIRECT SECOND
    // Now that the database is guaranteed to be updated, we send them to WhatsApp
    res.redirect(
      `https://wa.me/${activeNumber}?text=Hi,%20I%20want%20to%20create%20an%20ID.%20My%20promo%20code%20is%20${cleanPromo}`,
    );
  } catch (error) {
    console.error("Error in /join/:promoCode route:", error);
    // Safety check so the app doesn't crash if an error happens after redirect
    if (!res.headersSent) {
      res.redirect("/");
    }
  }
...
app.post("/vso/place-bet", async (req, res) => {
  try {
    if (!req.user) {
      return res
        .status(401)
        .json({ success: false, error: "Please login to place a bet." });
    }
    ...
    if (currentGameState.eventId !== eventId) {
      return res
        .status(400)
        .json({ success: false, error: "This match is no longer active." });
    }

    if (currentGameState.phase !== "betting" || currentGameState.timer <= 1) {
      return res.status(400).json({
        success: false,
        error: "Bets are closed! Wait for the next ball.",
      });
    }
    ...
    if (!numStake || numStake < 10) {
      return res
        .status(400)
        .json({ success: false, error: "Minimum bet amount is ₹10." });
    }
    ...
    if (selection === "IND_WIN") {
      ...
      return res
        .status(400)
        .json({ success: false, error: "Invalid bet selection." });
    }
    ...
    res
      .status(500)
      .json({ success: false, error: "Server error placing bet." });
  }
});
...
const matchAutomation = async () => {
  try {
    ...
    if (tossEvents.length > 0) {
      await Promise.all(
        tossEvents.map(async (event) => {
          event.tossMarket.status = "suspended";
          await event.save();
          console.log(
            `⏱️ [CRON] Automatically suspended Toss Market for: ${event.homeTeam} vs ${event.awayTeam}`,
          );
        }),
      );
    }
    ...
    if (startingEvents.length > 0) {
      await Promise.all(
        startingEvents.map(async (event) => {
          event.status = "live";
          await event.save();
          console.log(
            `⏱️ [CRON] Match status changed to LIVE (Starts in 30m): ${event.homeTeam} vs ${event.awayTeam}`,
          );
        }),
      );
    }
...
const settleEventBets = async () => {
  try {
    ...
    console.log(
      `Found ${eventsToSettle.length} event(s) to settle receipts for...`,
    );
    ...
    for (const event of eventsToSettle) {
      ...
      console.log(
        `✓ Settled Receipts for: ${event.homeTeam} vs ${event.awayTeam} | Bets Processed: ${pendingBets.length}`,
      );
    }
  } catch (error) {
...
const voidPendingBets = async () => {
  try {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `🧹 Sweeper found and marked ${result.modifiedCount} abandoned casino games as lost.`,
      );
    }
  } catch (err) {
...
app.use((err, req, res, next) => {
  ...
  let description =
    "Don't worry, your wallet balance and active bets are 100% safe. We are just experiencing a brief technical hiccup.";
